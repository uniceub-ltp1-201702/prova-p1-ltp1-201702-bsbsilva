...
public class BDSimulado {
	...
	// (2 A)  Crie uma função que retorne todas as informações de um Cliente fornecido o seu nome
	...
	///(2B) Crie uma função que retorne a lista de todas as Ligações realizadas por um Cliente determinado
	...
	//(2C)  Crie uma função que retorne o número de ligações realizadas por todos os Clientes de uma determinada UF
	...
	public int ligacoesPorUF(String UF){
		...
		for (int i = 0; i < this.ligacoes.size(); i++) {
			//Verificar ´passageritro é por nome
	...
	//(2D)  Crie uma função que retorne uma lista com os
	//códigos de todas as Ligações onde a UF de origem seja diferente da UF de destino
	...
	public ArrayList<Integer> ligacoesUFDiferente() {
	  ...
	  //(3A)  O resultado mais importante de um Sistema de Bilhetagem é a conta do Cliente. Crie um método 
	//(ou um conjunto deles) que permita emitir a conta de um determinado Cliente, mostrando seus dados pessoais e a 
	//lista de suas ligações, onde, para cada ligação, seja mostrada a data e a hora em que 
	  //a Ligação foi iniciada, o número de destino e a cidade/UF de destino.
